...
- **Expand SELECT \*** only resolves plain base tables/views (including system compatibility views like `sysobjects`) reachable through the query window's own connection, plus four-part linked-server references (`LinkedServer.Database.Schema.Table`) when the linked server is itself SQL Server — CTEs, derived tables, table-valued functions, temp tables, table variables, synonyms, and non-SQL-Server linked servers (Oracle, ODBC, flat file, ...) are left as `SELECT *`. SQL Server Authentication connections may not always be resolvable (Windows/Integrated Authentication is the reliable case); if the connection can't be determined at all, the command tells you and leaves the script untouched.
